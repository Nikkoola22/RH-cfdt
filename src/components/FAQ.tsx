    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 py-8 sm:py-12 px-2 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 space-y-4 animate-slide-up">
          ...
          {/* Category Filter */}
          <div className="flex flex-wrap items-center gap-2 p-4 bg-white rounded-2xl shadow-lg glass-card-light">
            <span className="text-sm font-semibold text-gray-700">Filtrer par:</span>
            <button
              onClick={() => setActiveCategory(null)}
              className={`px-4 py-2 text-sm font-semibold rounded-full transition-all duration-200 transform glass-pill ${activeCategory === null ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-lg scale-105' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
              ✓ Tous
            </button>
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold rounded-full transition-all duration-200 transform flex items-center gap-2 glass-pill ${
                  activeCategory === cat 
        ...
        <div className="space-y-4">
          ...
          {filtered.map((item: FAQItem, index: number) => (
            ...
            >
              ...
              >
                <div className={`bg-white shadow-md hover:shadow-xl p-5 sm:p-6 transform transition-all duration-300 glass-card-light ${openIds[item.id] ? 'ring-2 ring-blue-300/50' : 'hover:scale-[1.01]'}`}>
                  {/* Header */}
